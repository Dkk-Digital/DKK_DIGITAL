    ...
    <group>
      <group ref={pulseRef} position={[0.5, 0.6, -4.4]}>
        <Torus args={[3.8, 0.05, 12, 90]}>
          <meshBasicMaterial color={PALETTE.steelBlue} transparent opacity={0.22} />
        </Torus>
        <Torus args={[4.6, 0.03, 12, 90]}>
          <meshBasicMaterial color={PALETTE.mistBlue} transparent opacity={0.16} />
        </Torus>
      </group>

      <group ref={haloRef} position={[0.3, -0.2, -4.8]}>
        <Torus args={[5.3, 0.02, 12, 120]}>
          <meshBasicMaterial color={PALETTE.deepSlate} transparent opacity={0.1} />
        </Torus>
      </group>

      <group ref={funnelRef} position={[0, -0.5, -3]}>
        <Cylinder args={[2.4, 3.3, 0.8, 42, 1, true]}>
          <meshBasicMaterial color={PALETTE.deepSlate} wireframe transparent opacity={0.22} />
        </Cylinder>
        <Cylinder args={[1.5, 2.4, 0.8, 42, 1, true]} position={[0, -0.9, 0]}>
          <meshBasicMaterial color={PALETTE.steelBlue} wireframe transparent opacity={0.2} />
        </Cylinder>
        <Cylinder args={[0.75, 1.5, 1.0, 42, 1, true]} position={[0, -1.85, 0]}>
          <meshBasicMaterial color={PALETTE.mistBlue} wireframe transparent opacity={0.2} />
        </Cylinder>
        <Sphere args={[0.32, 20, 20]} position={[0, -2.5, 0]}>
          <meshBasicMaterial color={PALETTE.warmAmber} transparent opacity={0.88} />
        </Sphere>
      </group>
      ...
      <group ref={chartRef} position={[0, -4, -1.8]}>
        {[0.9, 1.2, 1.8, 2.6, 3.2, 3.9].map((height, i) => {
          const color = i < 2 ? PALETTE.deepSlate : i < 4 ? PALETTE.steelBlue : PALETTE.mistBlue;
          return (
        ...
        {trendPoints.map((point, i) => (
          <Sphere key={`point-${i}`} args={[0.12, 16, 16]} position={point}>
            <meshBasicMaterial color={PALETTE.lineMint} transparent opacity={0.85} />
          </Sphere>
        ))}

        <Box args={[1.8, 0.85, 0.24]} position={[4.8, 2.25, -0.2]}>
          <meshBasicMaterial color={PALETTE.warmAmber} transparent opacity={0.16} />
        </Box>
        <Box args={[1.4, 0.64, 0.24]} position={[4.8, 1.2, -0.2]}>
          <meshBasicMaterial color={PALETTE.steelBlue} transparent opacity={0.16} />
        </Box>
      </group>

      {[[-5.7, 1.8, -4], [5.9, -1.6, -3.8], [-6.2, -2.4, -3.1], [6.4, 2.3, -4.2]].map((position, i) => (
        <Sphere key={`ambient-${i}`} args={[0.22, 16, 16]} position={position}>
          <meshBasicMaterial color={i % 2 === 0 ? PALETTE.steelBlue : PALETTE.warmAmber} transparent opacity={0.35} />
        </Sphere>
      ))}
    </group>
  );
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: -1, pointerEvents: 'none' }}>
      <Canvas camera={{ position: [0, 0, 11], fov: 46 }}>
        <color attach="background" args={['#f5f9fc']} />
        <fog attach="fog" args={['#f5f9fc', 8, 30]} />
        <ambientLight intensity={0.55} color="#ffffff" />
        <pointLight intensity={1.0} position={[8, 5, 8]} color="#97b7c2" />
        <pointLight intensity={0.75} position={[-7, -4, 6]} color="#d8b394" />
        <pointLight intensity={0.45} position={[0, 7, -4]} color="#dfeaf0" />
      ...
      <div
        style={{
          ...
          height: '100%',
          background:
            'radial-gradient(circle at 14% 20%, rgba(79,109,122,0.13) 0%, rgba(255,255,255,0.04) 42%), radial-gradient(circle at 78% 74%, rgba(194,139,92,0.10) 0%, rgba(255,255,255,0.04) 46%), linear-gradient(180deg, rgba(255,255,255,0.42) 0%, rgba(255,255,255,0.91) 100%)',
          zIndex: -1,
        }}
      />
    </div>
